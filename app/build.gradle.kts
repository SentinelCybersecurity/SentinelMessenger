...
val sentinelRevision = 3

// SENTINEL: Use default debug keystore generated by Android Studio at $HOME/.android/debug.keystore
...
android {
  ...
  testOptions {
    ...
    unitTests {
      ...
      // SENTINEL: Reduce memory pressure by limiting the number of test classes per runner
      all { task ->
        task.forkEvery = 64
      }
    }
  ...
  packaging {
    jniLibs {
      ...
      // SENTINEL: Compress native libs by default as APK is not split on ABIs
  ...
  if (sentinelRevision < 0 || currentHotfixVersion < 0 || (sentinelRevision + currentHotfixVersion) >= maxHotfixVersions) {
    throw GradleException("Sentinel revision $sentinelRevision or Hotfix version $currentHotfixVersion out of range")
  }
  ...
  defaultConfig {
    val sourceVersionNameWithRevision = "${canonicalVersionName}-${sentinelRevision}"

    versionCode = (canonicalVersionCode * maxHotfixVersions) + sentinelRevision + currentHotfixVersion
    ...
    buildConfigField("String", "FDROID_UPDATE_URL", "\"https://github.com/SentinelCybersecurity/SentinelMessenger/releases/latest/download\"")

    vectorDrawables.useSupportLibrary = true

    // SENTINEL: BUILD_TIMESTAMP may be zero in debug builds.
    buildConfigField("long", "BUILD_TIMESTAMP_OR_ZERO", getLastCommitTimestamp() + "L")
    buildConfigField("String", "GIT_HASH", "\"${getGitHash()}\"")
    // SENTINEL: Ensure to add any new URLs to SignalServiceNetworkAccess.HOSTNAMES list
    ...
    // SENTINEL: Rely on the built-in variables FLAVOR and BUILD_TYPE instead of BUILD_*_TYPE
  ...
  productFlavors {
    create("store") {
      ...
      buildConfigField("boolean", "MANAGE_SENTINEL_UPDATES", "false")
    }

    create("website") {
      dimension = "distribution"
      buildConfigField("boolean", "MANAGE_SENTINEL_UPDATES", "true")
      isDefault = true
    }
  ...
  lint {
    ...
    if (ciEnabled) {
      // SENTINEL: disable slow "newer dependency available" lint checks; versions are pinned.
      disable += setOf(
        "GradleDependency",
...
dependencies {
  ...
  implementation(libs.sentinel.argon2) { artifact { type = "aar" } }
  implementation(libs.sentinel.native.utils)
