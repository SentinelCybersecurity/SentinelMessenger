...
class SentinelSocketQrScannerViewModel : ViewModel() {
  ...
  fun onQrScanned(data: String) {
    ...
    disposables += SentinelSocketQrScanRepository.lookupQrLink(data)
  ...
  fun onQrImageSelected(context: Context, uri: Uri) {
    ...
    disposables += SentinelSocketQrScanRepository.scanImageUriForQrCode(context, uri)
      .observeOn(AndroidSchedulers.mainThread())
      .subscribeBy { result ->
        _state.value = _state.value.copy(
          qrScanResult = result,
