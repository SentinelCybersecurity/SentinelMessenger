...
/**
 * Prompts the user to scan a SentinelSocket QR code. Uses the activity result to communicate the recipient that was found, or null if no valid deeplink were scanned.
 * See [Contract].
 */
class SentinelSocketQrScannerActivity : AppCompatActivity() {

  private val viewModel: SentinelSocketQrScannerViewModel by viewModels()
  ...
  class Contract : ActivityResultContract<Unit, SentinelSocket?>() {
    override fun createIntent(context: Context, input: Unit): Intent {
      return Intent(context, SentinelSocketQrScannerActivity::class.java)
    }

    override fun parseResult(resultCode: Int, intent: Intent?): SentinelSocket? {
      return if (resultCode == RESULT_OK) {
        SentinelSocket.parseLink(intent?.data!!)
      } else {
        null
      }
    }
...
fun Content(
  ...
  state: SentinelSocketQrScannerViewModel.ScannerState,
