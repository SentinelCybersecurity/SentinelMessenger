...
class UnifiedPushSettingsFragment : DSLSettingsFragment(R.string.NotificationDeliveryMethod__unifiedpush) {
  ...
  private val qrScanLauncher: ActivityResultLauncher<Unit> =
    registerForActivityResult(SentinelSocketQrScannerActivity.Contract()) { sentinelSocket ->
      if (sentinelSocket != null) {
        viewModel.setSentinelSocket(sentinelSocket)
      }
    }
  ...
  private fun getConfiguration(state: UnifiedPushSettingsState): DSLConfiguration {
    return configure {
      ...
      clickPref(
        title = DSLSettingsText.from(R.string.UnifiedPushSettingsFragment__sentinelsocket_server),
        summary = DSLSettingsText.from(
          if (state.airGapped) {
            getString(R.string.UnifiedPushSettingsFragment__sentinelsocket_server_air_gapped)
          } else {
            state.sentinelSocketUrl ?: "Error"
          }
        ),
        onClick = {
      ...
      if (state.airGapped) {
        ...
        clickPref(
          ...
          summary = DSLSettingsText.from(getString(R.string.UnifiedPushSettingsFragment__tap_to_request_a_test_notification_from_sentinelsocket)),
          onClick = {
            viewModel.pingSentinelSocket()
  ...
  private fun getStatusSummary(state: UnifiedPushSettingsState): Int {
    return when {
      ...
      state.sentinelSocketUrl == null && !state.airGapped -> R.string.UnifiedPushSettingsFragment__status_summary_sentinelsocket_url_missing
