...
class UnifiedPushRefreshJob private constructor(
  ...
  public override fun onRun() {
    ...
    try {
      ...
      when (newStatus) {
        ...
        RegistrationStatus.FORBIDDEN_ENDPOINT -> UnifiedPushNotificationBuilder(context).setNotificationSentinelSocketForbiddenEndpoint()
        RegistrationStatus.FORBIDDEN_UUID -> UnifiedPushNotificationBuilder(context).setNotificationSentinelSocketForbiddenUuid()
        RegistrationStatus.FORBIDDEN_PASSWORD -> UnifiedPushNotificationBuilder(context).setNotificationSentinelSocketForbiddenPassword()
        else -> {}
      }
    } catch (t: Throwable) {
  ...
  private fun checkRegistrationStatus(): RegistrationStatus {
    ...
    if (!airGapped && sentinelSocketUrl == null) {
      Log.e(TAG, "Missing SentinelSocket URL.")
      return RegistrationStatus.PENDING
    }
    ...
    Log.d(TAG, "Re-registering $device on SentinelSocket server...")

    val result = SentinelSocketRepository.registerDeviceOnServer(
      url = sentinelSocketUrl!!,
      device = device,
      endpoint = endpoint,
      ping = testPing,
    )
