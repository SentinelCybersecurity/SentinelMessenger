...
public class TextSecurePreferences {
  ...
  private static final String[] booleanPreferencesToBackupSentinel = {
      LOG_ENABLED,
      UPDATE_APK_ENABLED,
  ...
  private static final String[] stringSetPreferencesToBackupSentinel = {PASSPHRASE_LOCK_TRIGGER};

  private static final String[] integerPreferencesToBackupSentinel = {PASSPHRASE_LOCK_TIMEOUT};
  ...
  public static long getPreferencesToSaveToBackupCount(@NonNull Context context) {
    ...
    for (String booleanPreference : booleanPreferencesToBackupSentinel) {
      if (preferences.contains(booleanPreference)) {
        count++;
      }
    }

    for (String stringSetPreference : stringSetPreferencesToBackupSentinel) {
      if (preferences.contains(stringSetPreference)) {
        count++;
      }
    }

    for (String integerPreference : integerPreferencesToBackupSentinel) {
  ...
  public static int getLastVersionCodeForSentinel(Context context) {
    return getIntegerPreference(context, LAST_VERSION_CODE_PREF, BuildConfig.VERSION_CODE);
  }

  public static void setLastVersionCodeForSentinel(Context context, int versionCode) {
